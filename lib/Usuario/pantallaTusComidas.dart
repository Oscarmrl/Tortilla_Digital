...
class MisComidasScreen extends StatefulWidget {
  ...
  const MisComidasScreen({this.userId = 'usuario11', Key? key})
    : super(key: key);
...
class _MisComidasScreenState extends State<MisComidasScreen> {
  ...
  Future<void> agregarRecetaAlHistorial(String recetaId) async {
    ...
    await FirebaseFirestore.instance.runTransaction((transaction) async {
      ...
      List historial = snapshot.data()?['recetasHistorial'] ?? [];

      // Agregar nueva receta
      historial.add({'idReceta': recetaId, 'fechavista': Timestamp.now()});
      ...
      transaction.update(userRef, {'recetasHistorial': historial});
    });
  ...
  Future<List<Map<String, dynamic>>> obtenerHistorialOrdenado() async {
    ...
    historial.sort(
      (a, b) => (b['fechavista'] as Timestamp).compareTo(
        a['fechavista'] as Timestamp,
      ),
    );
              ...
              builder: (context, snapshot) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final fecha = _formatTimestamp(receta['fechavista']);
                    return ListTile(
